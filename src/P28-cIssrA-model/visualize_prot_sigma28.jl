...
function plot_prot_sigma28_data(time_array, simulation_data_array,experimental_data_dictionary)

    (NR,NC) = size(time_array)
    for index = 1:20:NC
        plot(time_array[:,index],simulation_data_array[:,index],color="#7AD7F0",alpha=0.80,lw=0.5)
    end
    ...
    # TEXP = experimental_data_dictionary["deGFP_protein_data_table"][!,:Time_in_hr]
    # DATA = experimental_data_dictionary["deGFP_protein_data_table"][!,:Concentration_in_muM]
    # STD = (2.58/sqrt(5))*experimental_data_dictionary["deGFP_protein_data_table"][!,:stdev]
    ...
    ylabel(L"Concentration ($\mu$M)",fontsize=16)
    ...
    # wwrite -
    savefig("./plots/Prot-sigma28-Ensemble.pdf", facecolor=fig.get_facecolor(), edgecolor="none")
end
...
function main(path_to_simulation_dir::String, path_to_plot_file::String)
    ...
    # plot -
    plot_prot_sigma28_data(time_array, data_array, exp_dd)
end
