...
function plot_mRNA_deGFP_data(time_array, simulation_data_array,experimental_data_dictionary)
    ...
    TEXP = experimental_data_dictionary["deGFP_mRNA_data_table"][!,:Time_in_hr]
    DATA = experimental_data_dictionary["deGFP_mRNA_data_table"][!,:Concentration_in_nM]
    STD = (2.58/sqrt(8))*experimental_data_dictionary["deGFP_mRNA_data_table"][!,:stdev]
    ...
    xlim([0.5,17])
     ...
     # write -
     savefig("./plots/mRNA-deGFP-Ensemble-Inset.pdf", facecolor=fig.get_facecolor(), edgecolor="none")
end
...
function main(path_to_simulation_dir::String, path_to_plot_file::String)
    ...
    # plot -
    plot_mRNA_deGFP_data(time_array, data_array, exp_dd)
end
