...
function precompute_transcription_kinetic_limit_array_cell_free(biophysical_dictionary::Dict{String,Any}, gene_coding_length_array::Array{Float64,1}, gene_abundance_array::Array{Float64,1},  time_constant_modifier_array::Array{Float64,1})::Array{Float64,1}
    ...
    epsilon_array = Float64[]
    kI = (1/characteristic_initiation_time)

    # gene 1
    G1 = gene_abundance_array[1]
    gene_length_1 = gene_coding_length_array[1]
    kE = transcription_elongation_rate*(1/gene_length_1)
    time_scale_parameter = time_constant_modifier_array[5]
    tau_factor_1 = (kE/kI)*time_scale_parameter

    # gene 2
    G2 = gene_abundance_array[2]
    gene_length_2 = gene_coding_length_array[2]
    kE = transcription_elongation_rate*(1/gene_length_2)
    time_scale_parameter = time_constant_modifier_array[6]
    tau_factor_2 = (kE/kI)*time_scale_parameter

    # gene 3
    G3 = gene_abundance_array[3]
    gene_length_3 = gene_coding_length_array[3]
    kE = transcription_elongation_rate*(1/gene_length_3)
    time_scale_parameter = time_constant_modifier_array[7]
    tau_factor_3 = (kE/kI)*time_scale_parameter

    # epsilon -
    epsilon_1 = (tau_factor_1/tau_factor_2)*(1+tau_factor_2)*G2 + (tau_factor_1/tau_factor_3)*(1+tau_factor_3)*G3   # j = 1 (i=2,3)
    epsilon_2 = (tau_factor_2/tau_factor_1)*(1+tau_factor_1)*G1 + (tau_factor_2/tau_factor_3)*(1+tau_factor_3)*G3   # j = 2 (i=1,3)
    epsilon_3 = (tau_factor_3/tau_factor_1)*(1+tau_factor_1)*G1 + (tau_factor_3/tau_factor_2)*(1+tau_factor_2)*G2   # j = 3 (i=1,2)
    push!(epsilon_array, epsilon_1)
    push!(epsilon_array, epsilon_2)
    push!(epsilon_array, epsilon_3)
    push!(epsilon_array, epsilon_3)
...
function calculate_translation_kinetics_array(t,x,mRNA_index_array,translation_parameters_array, data_dictionary)
    ...
    # gene 1
    M1 = x[5]
    parameters_struct = translation_parameters_array[1]
    tau_factor_1 = parameters_struct.tau_factor
    KL_1 = parameters_struct.KL

    # gene 2
    M2 = x[6]
    parameters_struct = translation_parameters_array[2]
    tau_factor_2 = parameters_struct.tau_factor
    KL_2 = parameters_struct.KL

    # gene 3
    M3 = x[7]
    parameters_struct = translation_parameters_array[3]
    tau_factor_3 = parameters_struct.tau_factor
    KL_3 = parameters_struct.KL

    # compute epsilon -
    epsilon_1 = (KL_1*tau_factor_1/KL_2*tau_factor_2)*(1+tau_factor_2)*M2 + (KL_1*tau_factor_1/KL_3*tau_factor_3)*(1+tau_factor_3)*M3   # j = 1 (i=2,3)
    epsilon_2 = (KL_2*tau_factor_2/KL_1*tau_factor_1)*(1+tau_factor_1)*M1 + (KL_2*tau_factor_2/KL_3*tau_factor_3)*(1+tau_factor_3)*M3   # j = 2 (i=1,3)
    epsilon_3 = (KL_3*tau_factor_3/KL_1*tau_factor_1)*(1+tau_factor_1)*M1 + (KL_3*tau_factor_3/KL_2*tau_factor_2)*(1+tau_factor_2)*M2   # j = 3 (i=1,2)
    push!(epsilon_array, epsilon_1)
    push!(epsilon_array, epsilon_2)
    push!(epsilon_array, epsilon_3)
    push!(epsilon_array, epsilon_3)
    ...
    for index = 1:number_of_translation_rates
        ...
        # build the rate -
        #value = vmax*(mRNA_concentration/(KL*tau+(1+tau)*mRNA_concentration) + epsilon_array[index])
